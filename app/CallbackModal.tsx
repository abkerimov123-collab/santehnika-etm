...
export default function CallbackModal({ open, phone, phoneDisplay, onClose }: Props) {
  ...
  useEffect(() => {
    if (!open) { setStatus('idle'); setUserPhone(''); setConsent(false); setShowHint(false); setHighlight(false) }
  }, [open])
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      params.append('FIELDS[PHONE][0][VALUE]', userPhone.trim())
  ...
  return (
    <div className="cm-overlay" onClick={onClose} role="dialog" aria-modal="true" aria-label="Свяжитесь с нами">
      <div className="cm-dialog" onClick={e => e.stopPropagation()}>
        <div className="cm-header">
          <span className="cm-logo">сантехника <LogoIcon size={18} /> етм</span>
            ...
            <div className="cm-channels">
              <a href={`tel:${phone}`} className="cm-channel cm-channel--call">
                ...
                <span className="cm-channel-info">
                  ...
                  <span className="cm-channel-hint">{phoneDisplay}</span>
                </span>
